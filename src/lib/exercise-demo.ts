...
// Server-side only — reads the bundled exercise DB JSON
...
const CUSTOM_OVERRIDES: Record<string, Partial<ExerciseDemoResult>> = {
  "pike push-up": {
    ...
    customAnimationKey: "pike-push-up",
  },
  "hands-elevated pike push-up": {
    name: "Hands-Elevated Pike Push-Up",
    level: "Beginner",
    equipment: "Box or Bench (18-24 inches)",
    primaryMuscles: ["Anterior Deltoid", "Triceps Brachii"],
    secondaryMuscles: ["Core", "Serratus Anterior"],
    instructions: [
      "Place your hands on an elevated surface (bench or box) slightly wider than shoulder-width.",
  ...
  "hollow body hold": {
    ...
    hasCustomAnimation: true,
    customAnimationKey: "hollow-body",
  },
  "standard push-up": {
    name: "Push-Up",
    level: "Beginner",
...
export function getExerciseDemo(exerciseName: string): ExerciseDemoResult {
  ...
  if (custom) {
    return {
      found: true,
      dbName: null,
      name: custom.name ?? exerciseName,
      level: custom.level ?? "Intermediate",
